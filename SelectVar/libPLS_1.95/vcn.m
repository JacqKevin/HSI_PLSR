...
while i<=N
    ...
    Xcal=X(:,vsel);
    %+++ cross validation
    CV=plscv0(Xcal,ycal,A,K,method);
    PLS=pls(Xcal,ycal,CV.optLV,method);
    coef(i,vsel)=PLS.regcoef_original(1:end-1)';
    PredError(i)=CV.RMSECV_min;    %revision
    nLV(i)=CV.optLV;          %revison
    if rem(i,100)==0;
        %fprintf('The %d/%dth sampling for variable selection finished.\n',i,N);
    end
    i=i+1;
end
